...
/*
    ...
    Place the "rm_layout_N" room first to run this. Each instance on a layer will
    be converted to a string using ds_list_write() which can be converted back to a list using ds_list_read().
    Then each list of instances will be output to the debug window that can be copied directly into a GML script
    to randomly select the list of instances to add to the scenes.
    ...
    Game Maker assigns object_index when the game is compiled and can changed during development.
